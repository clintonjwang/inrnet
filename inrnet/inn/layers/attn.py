...
# class SelfAttnINRLayer(nn.Module):
...
#     def __init__(self, in_ch, out_ch, num_samples, d_k, V_dim, spatial_dim=2):
...
#         super().__init__()
#         self.d_k = d_k
#         self.QK_coords = nn.Parameter(torch.rand(num_samples, d_k))
#         self.K_weights = nn.Parameter(torch.randn(num_samples, d_k))
#         self.Q_weights = nn.Parameter(torch.randn(num_samples, d_k))
...
class MultiHeadSelfAttnINR(nn.Module):

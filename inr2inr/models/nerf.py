...
class PaperNeRFModel(nn.Module):
    ...
    def __init__(
        ...
        self.layers_xyz.append(nn.Linear(self.dim_xyz, 256))
        for i in range(1, 8):
            if i == 4:
                self.layers_xyz.append(nn.Linear(self.dim_xyz + 256, 256))
            else:
                self.layers_xyz.append(nn.Linear(256, 256))
        self.fc_feat = nn.Linear(256, 256)
        self.fc_alpha = nn.Linear(256, 1)

        self.layers_dir = nn.ModuleList()
        self.layers_dir.append(nn.Linear(256 + self.dim_dir, 128))
        for i in range(3):
            self.layers_dir.append(nn.Linear(128, 128))
    ...
    def __init__(
        ...
        if self.use_viewdirs:
            ...
            self.layers_dir.append(
                nn.Linear(self.dim_dir + hidden_size, hidden_size // 2)
            )
